class graphHandler {
    constructor() {
        this.microbitHandlers=[];
        this.headersList=[];
        this.headers = ["time"];
        this.data = [];
        this.dataParsedUpto=[];
        this.graph = new lineGraph(this.headers,[]);
    }

    addDevice(device){
        this.microbitHandlers.push(device);
    ...
    checkHeaders(){
        for(let i=0;i<this.microbitHandlers.length;i++){
            let correctHeaders=this.microbitHandlers[i].headers;
            for(let headerI=1;headerI<correctHeaders.length;headerI++){
                let headerName = this.microbitHandlers[i].getName()+correctHeaders[headerI];
                if(!(this.headers.includes(headerName))){
                    ...
                    if(this.graph){
                        this.graph.addHeader(headerName)
                    }
                }
            }
        }
    ...
    update(){
        if(this.microbitHandlers.length==0){
            return;
        }
        ...
        if(this.graph){
            this.graph.updateGraphData(this.data);
        }
    }
}
...
// class lineChartOld extends graphBase{
//     constructor(title="",headers=[],type=""){
//         super(title,headers,type);
